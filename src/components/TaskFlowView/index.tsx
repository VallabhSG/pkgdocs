...
function TaskCard({ task }: { task: Task }) {
  ...
  const activeCode = activeStep !== null ? task.steps[activeStep].code : task.steps.map((s) => s.code).join("\n");
                ...
                {activeStep !== null && (
                  ...
                  >
                    <div className="bg-slate-800 px-4 py-2 text-xs text-slate-400 font-mono flex items-center gap-2">
                      ...
                      <span className="ml-1 flex-1">{task.steps[activeStep].label}</span>
                      <CopyButton text={task.steps[activeStep].code} />
                    </div>
              ...
              {activeStep === null && (
                <div className="rounded-xl overflow-hidden">
                  <div className="bg-slate-800 px-4 py-2 text-xs text-slate-400 font-mono flex items-center gap-2">
                    ...
                    <span className="w-2.5 h-2.5 rounded-full bg-yellow-500/60" />
                    <span className="w-2.5 h-2.5 rounded-full bg-green-500/60" />
                    <span className="ml-1 flex-1">Full recipe — click a step above for detail</span>
