...
const CATEGORIES: { label: string; emoji: string; ids: string[] }[] = [
  ...
  { label: "HTTP and Async",  emoji: "⚡", ids: ["requests", "httpx", "axios", "ky", "aiohttp", "anyio"] },
  { label: "Testing",         emoji: "🧪", ids: ["pytest", "vitest", "playwright"] },
  { label: "ORM and Database",emoji: "🗄️", ids: ["prisma", "drizzle-orm", "sqlalchemy", "redis-py", "alembic"] },
  { label: "State and Forms", emoji: "🔄", ids: ["zustand", "jotai", "immer", "react-query", "react-hook-form"] },
  { label: "Validation",      emoji: "✅", ids: ["zod", "pydantic"] },
  { label: "Scraping and ML", emoji: "🤖", ids: ["scrapy", "beautifulsoup4", "boto3"] },
  { label: "Build Tooling",   emoji: "🔧", ids: ["vite", "tailwindcss", "typescript"] },
  { label: "CLI and Output",  emoji: "🖥️", ids: ["click", "typer", "rich"] },
];

const FEATURES = [
  { num: "01", title: "Story View", desc: "The problem, mental model, and when to use it — written for humans." },
...
Set-Content -Path 'e:\Java WorkSpace\pkgdocs\src\app\page.tsx' -Encoding UTF8 -Value $content
Write-Output "done"
